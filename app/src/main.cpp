...
#include "mgos_gpio.h"
#include "mgos_adc.h" //adc

#define GPIO0 0 
#define WATER_SENSOR_GPIO32 32
#define GAS_SENSOR_GPIO33 33  
#define MOTION_SENSOR_GPIO13 13
...
//  LOG(LL_INFO,("{Hum: %f}", bme->getHumidity()));
  ...
  //output ADC pin values:
  LOG(LL_INFO, ("Water Sensor: %d",mgos_adc_read(WATER_SENSOR_GPIO32)));
  LOG(LL_INFO, ("Gas Sensor: %d",mgos_adc_read(GAS_SENSOR_GPIO33)));
  //LOG(LL_INFO, ("Motion Sensor: %d",mgos_adc_read(MOTION_SENSOR_GPIO13)));
  LOG(LL_INFO, ("Motion Sensor: %d",mgos_gpio_read(MOTION_SENSOR_GPIO13)));
...
//    LOG(LL_INFO, ("setting apmode"));
    ...
    if (mgos_sys_config_get_apmode()) {
      ...
      ap_cfg.enable = true;
	  sprintf(ap_cfg.ssid,"SafetySensor");
...
//  mgos_gpio_set_button_handler(USER_BUTTON_GPIO, MGOS_GPIO_PULL_UP, MGOS_GPIO_INT_EDGE_NEG, 20, button_USER_cb, NULL);
  mgos_gpio_set_button_handler(GPIO0, MGOS_GPIO_PULL_UP, MGOS_GPIO_INT_EDGE_NEG, 20, button_USER_cb, NULL);
...
*/
    ...
    //enable ADC pins
    mgos_gpio_set_mode(WATER_SENSOR_GPIO32,MGOS_GPIO_MODE_INPUT);
    mgos_adc_enable(WATER_SENSOR_GPIO32);
    mgos_gpio_set_mode(GAS_SENSOR_GPIO33,MGOS_GPIO_MODE_INPUT);
    mgos_adc_enable(GAS_SENSOR_GPIO33);
    mgos_gpio_set_mode(MOTION_SENSOR_GPIO13,MGOS_GPIO_MODE_INPUT);
    //mgos_adc_enable(MOTION_SENSOR_GPIO13);

//see https://github.com/mongoose-os-libs/adc/blob/master/esp32/src/esp32_adc.c
//mgos_adc_read
//mgos_adc_read_voltage

    //call timer cb every 10minutes
    //mgos_set_timer(600000, MGOS_TIMER_REPEAT, mqtt_timer_cb, NULL);
    //call timer call back every second
    mgos_set_timer(1000, MGOS_TIMER_REPEAT, mqtt_timer_cb, NULL);
